# Product Hunt Launch Kit: 5e Character Vault

Prepared: 2026-05-19

## Product Basics

- Product name: `5e Character Vault`
- Product URL: `https://yudin-s.github.io/dnd-character-vault/`
- Source code: `https://github.com/yudin-s/dnd-character-vault`
- Maker: Sergey Yudin, `https://github.com/yudin-s`
- Pricing: Free
- License: MIT
- Suggested topics: `Open Source`, `Games`, `Productivity`, `Developer Tools`, `No-Code`, `Design Tools`

## Product Hunt Copy

Tagline options:

1. `A local-first DnD 5e character sheet`
2. `A browser-only vault for DnD 5e characters`
3. `Run DnD 5e sessions from one local-first sheet`
4. `A privacy-friendly character sheet for DnD 5e`
5. `DnD 5e characters, dice, and backups in your browser`

Recommended tagline:

`A local-first DnD 5e character sheet`

Short description:

`5e Character Vault is a browser-only DnD 5e sheet for live sessions: track HP, resources, inventory, spells, dice rolls, autosaved history, and JSON backups without creating an account.`

Product Hunt description under 260 characters:

`A browser-only DnD 5e sheet for live sessions. Track HP, resources, inventory, spells, dice rolls, autosaved history, and portable JSON backups locally, without accounts or a server.`

One-liner for social:

`I built 5e Character Vault: a free, local-first DnD 5e character sheet that runs entirely in the browser.`

Maker first comment:

```text
Hi Product Hunt,

I built 5e Character Vault because I wanted a DnD 5e sheet that feels useful at the table, not just complete on paper.

It runs entirely in the browser: no account, no backend, local autosave, JSON backups, quick dice rolls, session controls, spells, inventory, and history snapshots.

The current focus is practical play: big HP controls, resources, death saves, conditions, dice, and a compact edit mode for the full sheet.

I would love feedback on:
- what is still too slow during a live session
- which 5e workflows should be one tap
- whether the layout feels better on mobile or desktop

It is open source and free under MIT.
```

## Required Listing Assets

Product Hunt's own posting guide asks for product URL, product name, tagline, topics, thumbnail, pricing tag, gallery, description, makers, optional YouTube video/demo, and first comment. It recommends a `240x240` thumbnail and `1270x760` gallery images; the gallery needs at least `2` images before it is viewable.

Asset checklist:

| Asset | Size / format | Status | File / action |
| --- | --- | --- | --- |
| Product logo / thumbnail | 240x240 PNG or GIF under 3 MB | Ready | `docs/product-hunt/assets/product-hunt-logo-240.png` |
| Gallery image 1: hero | 1270x760 PNG/JPG | Needed | Screenshot + headline |
| Gallery image 2: live session controls | 1270x760 PNG/JPG | Needed | HP/resources/dice area |
| Gallery image 3: spellbook | 1270x760 PNG/JPG | Needed | Spellbook desktop view |
| Gallery image 4: inventory + coins | 1270x760 PNG/JPG | Needed | Inventory edit/play view |
| Gallery image 5: local backups/history | 1270x760 PNG/JPG | Needed | Floating history drawer + export/import |
| Gallery image 6: mobile layout | 1270x760 PNG/JPG | Needed | Mobile screenshot in device frame |
| Optional gallery video | YouTube full URL, 16:9 | Recommended | 45-60 second walkthrough |
| Interactive demo | Product Hunt-supported demo link | Optional | Arcade, Supademo, Storylane, etc. |
| Social OG image | 1200x630 PNG/JPG | Needed | Use hero gallery visual adapted to 1.91:1 |
| X/Twitter image | 1600x900 PNG | Needed | Hero visual + launch copy |
| LinkedIn image | 1200x627 PNG | Needed | Hero visual + clearer text |
| Maker avatar | Square profile image | Needed in PH profile | Use real maker photo |

Gallery copy:

1. Hero:
   `A local-first DnD 5e character sheet`
   `Run your session from the browser, without accounts or a server.`

2. Session cockpit:
   `Everything needed during play`
   `HP, resources, death saves, conditions, quick rolls, and XP stay close.`

3. Dice:
   `Physics dice, built in`
   `Roll d4-d20 directly from the sheet.`

4. Spells:
   `A spellbook that works at the table`
   `Slots, prepared spells, filters, casting details, and notes.`

5. Local-first safety:
   `Your character stays with you`
   `Autosave history, restore points, JSON export, and import.`

6. Mobile:
   `Designed for desktop and mobile sessions`
   `Use it beside your map, notes, or VTT.`

## Demo Video Script

Length: 45-60 seconds.

Structure:

1. 0-5s: open with the play dashboard.
   Voiceover: `This is 5e Character Vault, a local-first character sheet for DnD 5e.`

2. 5-15s: show HP, damage, healing, XP, resources.
   Voiceover: `The play mode keeps table-critical controls large and close: HP, resources, XP, death saves, and conditions.`

3. 15-25s: open dice modal and roll.
   Voiceover: `Dice are built in, including physical 3D rolls for common 5e dice.`

4. 25-38s: show edit mode, spellbook, inventory.
   Voiceover: `The full sheet is still there: identity, combat, inventory, coins, resources, spells, and notes.`

5. 38-50s: open floating history and export.
   Voiceover: `Everything stays in the browser with autosave history and portable JSON backups.`

6. 50-60s: close on URL.
   Voiceover: `It is free, open source, and ready to try.`

## Launch Content Plan

### 7-5 Days Before Launch

- Create Product Hunt maker profile with real name, photo, and GitHub/social links.
- Prepare all gallery images and test the first image at 240x180 crop.
- Record the demo video and upload it to YouTube as unlisted.
- Ask 5-10 DnD players or GMs to test the live URL and report blockers.
- Make sure the GitHub README explains local-first storage and backup.

### 3 Days Before Launch

- Create a Product Hunt draft with all copy and visuals.
- Verify the product URL works in a clean browser profile.
- Prepare personal outreach messages for friends, makers, DnD groups, and open-source contacts.
- Write a short changelog post on GitHub or social.

### 1 Day Before Launch

- Final screenshot pass on desktop and mobile.
- Confirm links: live app, GitHub source, license, author.
- Prepare launch posts for X, LinkedIn, Reddit/Discord communities where self-promotion is allowed.
- Sleep; launch day is mostly replies.

### Launch Day

- Publish at 12:01 AM Pacific if scheduling on Product Hunt.
- Post the maker first comment immediately.
- Share the launch with context, not just a bare link.
- Reply to every comment with substance.
- Track bugs in GitHub issues and ship small fixes quickly.

### Day After

- Thank supporters.
- Publish a short results/learning post.
- Convert repeated feedback into GitHub issues.
- Add Product Hunt badge only if the launch gets meaningful traction.

## Launch Posts

X/Twitter:

```text
I launched 5e Character Vault on Product Hunt today.

It is a free, open-source, local-first DnD 5e character sheet:
- live session controls
- spells, inventory, resources
- built-in dice
- autosave history
- JSON backups
- no account, no backend

Feedback from DnD players and GMs would mean a lot:
{PRODUCT_HUNT_URL}
```

LinkedIn:

```text
Today I am launching 5e Character Vault on Product Hunt.

It is a browser-only DnD 5e character sheet designed around live sessions: fast HP controls, resources, spells, inventory, dice rolls, autosaved history, and portable JSON backups.

The main design constraint was privacy and durability: no account, no server, no lock-in. Your character data stays in your browser unless you export it.

The project is free and open source under MIT.

I would appreciate feedback from tabletop players, GMs, and product/design folks:
{PRODUCT_HUNT_URL}
```

Discord / community version:

```text
I built a free local-first DnD 5e character sheet and launched it on Product Hunt.

It runs fully in the browser and focuses on actual table use: HP, resources, spells, inventory, dice, history snapshots, and JSON backups.

No account or server. Feedback from players/GMs is very welcome:
{PRODUCT_HUNT_URL}
```

## Positioning

Primary angle:

`A practical DnD 5e sheet for live play, not another account-gated character database.`

Differentiators:

- Local-first: character data stays in the browser.
- No account required.
- Built-in dice and session controls.
- Autosave history and manual JSON backups.
- Open source under MIT.
- Desktop and mobile friendly.

Potential objections:

- `Is this official D&D Beyond?`
  No. It is an independent, open-source 5e utility.

- `Will my character sync across devices?`
  Not yet. The current model is local-first with JSON export/import.

- `Will data survive browser cleanup?`
  Browser storage can be cleared by the user or browser. Export JSON backups for safety.

## Sources

- Product Hunt Help Center, `How to post a product`: `https://help.producthunt.com/en/articles/479557-how-to-post-a-product`
- Product Hunt Help Center, `Featuring Guidelines`: `https://help.producthunt.com/en/articles/9883485-product-hunt-featuring-guidelines`
- Screenhance, `Product Hunt Launch Checklist 2026`: `https://screenhance.com/product-hunt-launch-checklist`
